...
impl ModelClientSession {
    ...
    pub async fn stream(
        ...
        match wire_api {
            ...
            WireApi::Chat => {
                // Build a ResponsesApiRequest the same way as for the Responses API,
                // then delegate to the chat_completions translation layer.
                let client_setup = self.client.current_client_setup().await?;
                let request = self.build_responses_request(
                    &client_setup.api_provider,
                    prompt,
                ...
                let auth_manager = &self.client.state.auth_manager;

                crate::chat_completions::stream_chat_completions(
                    &request, provider, auth_manager,
                )
                .await
            }
        }
