...
impl ModelsManager {
    ...
    pub fn new(codex_home: PathBuf, auth_manager: Arc<AuthManager>) -> Self {
        Self::new_with_provider(codex_home, auth_manager, ModelProviderInfo::create_openai_provider())
    }

    /// Construct a manager with a specific model provider.
    ///
    /// Use this when a custom provider (e.g. Synthetic, OpenRouter, LM Studio)
    /// is selected so the model picker queries the correct `/models` endpoint.
    pub fn new_with_provider(
        ...
        let is_custom_provider = !provider.is_openai();
        Self {
            local_models: if is_custom_provider {
                Vec::new() // Don't mix OpenAI built-in presets with custom provider models
            } else {
                builtin_model_presets(auth_manager.auth_mode())
            },
            remote_models: RwLock::new(if is_custom_provider {
                Vec::new() // Start empty — will be populated from the provider's /models
            } else {
    ...
    async fn refresh_available_models(
        ...
        // For custom (non-OpenAI) providers, always try to fetch from their
        // /models endpoint regardless of feature flags or auth mode.
        let is_custom_provider = !self.provider.is_openai();

        if !is_custom_provider && !config.features.enabled(Feature::RemoteModels) {
            return Ok(());
        }
        if !is_custom_provider && self.auth_manager.auth_mode() != Some(AuthMode::Chatgpt) {
            if matches!(
                refresh_strategy,
        ...
        match refresh_strategy {
            ...
            RefreshStrategy::OnlineIfUncached => {
                ...
                if !is_custom_provider && self.try_load_cache().await {
    ...
    async fn fetch_and_update_models(&self) -> CoreResult<()> {
        ...
        // Custom providers use the standard OpenAI-compatible `/v1/models`
        // endpoint which returns `{"data": [{"id": "…", …}]}`.  The built-in
        // ModelsClient expects the Codex-specific `{"models": […]}` shape, so
        // we use a lightweight direct fetch instead.
        if !self.provider.is_openai() {
            return self.fetch_custom_provider_models().await;
        }
    ...
    /// Fetch models from a custom (non-OpenAI) provider's `/v1/models` endpoint.
    ///
    /// These providers return the standard OpenAI format:
    ...
    async fn fetch_custom_provider_models(&self) -> CoreResult<()> {
        ...
        // Add bearer token from the provider's env_key if available.
        if let Ok(Some(api_key)) = self.provider.api_key() {
            req = req.header("Authorization", format!("Bearer {}", api_key));
        }

        let response = timeout(MODELS_REFRESH_TIMEOUT, req.send())
            .await
            .map_err(|_| CodexErr::Timeout)?
            .map_err(|e| CodexErr::Fatal(format!("Failed to fetch models from {url}: {e}")))?;

        let body = response
            .bytes()
            .await
            .map_err(|e| CodexErr::Fatal(format!("Failed to read model response body: {e}")))?;

        // Parse the standard OpenAI response format: {"data": [{"id": "…", …}]}
        ...
        let parsed: OpenAiModelsResponse =
            serde_json::from_slice(&body).map_err(|e| {
                CodexErr::Fatal(format!(
                    "Failed to parse models from {url}: {e}; body: {}",
                    String::from_utf8_lossy(&body)
                ))
            })?;

        // Convert to ModelInfo entries.
        let models: Vec<ModelInfo> = parsed
            ...
            .map(|(idx, m)| {
                ModelInfo {
                    ...
                    truncation_policy: TruncationPolicyConfig::bytes(200_000),
