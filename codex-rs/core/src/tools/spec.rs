...
fn create_task_dispatch_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "agent_name".to_string(),
            JsonSchema::String {
                description: Some(
                    "Name of the target agent in the LaborMarket to dispatch the task to.".to_string(),
                ),
            },
        ),
        (
            ...
            JsonSchema::String {
                description: Some(
                    "The task description / prompt to send to the subagent.".to_string(),
                ),
            },
        ),
    ]);
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Dispatch a task to a named subagent registered in the LaborMarket. \
            The agent will receive an isolated context with its system prompt and the task description. \
            Use create_subagent first to register agents, or use pre-registered agents."
            .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
            required: Some(vec!["agent_name".to_string(), "task".to_string()]),
            additional_properties: Some(false.into()),
        },
    })
}

fn create_create_subagent_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "name".to_string(),
            JsonSchema::String {
                description: Some(
                    "Unique name for the new agent (used as identifier for task_dispatch).".to_string(),
                ),
            },
        ),
        (
            ...
            JsonSchema::String {
                description: Some(
                    "Human-readable description of the agent's purpose and specialization.".to_string(),
                ),
            },
        ),
        (
            ...
            JsonSchema::String {
                description: Some(
                    "System prompt that defines the agent's behavior, expertise, and constraints.".to_string(),
                ),
            },
        ),
    ]);
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Create a new specialist subagent and register it in the LaborMarket. \
            Once created, the agent is immediately available for task_dispatch. \
            Dynamic agents cannot themselves create or dispatch to other agents."
            .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
...
fn create_feature_pipeline_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            ...
            JsonSchema::String {
                description: Some(
                    "Markdown implementation plan with ## Feature Name headings. \
                     Each heading defines a feature to process through the TDD pipeline."
                        .to_string(),
                ),
            },
        ),
        (
            "max_iterations_per_loop".to_string(),
            JsonSchema::Number {
                description: Some(
                    "Optional: maximum iterations for any single loop phase (default 25)."
                        .to_string(),
                ),
            },
        ),
        (
            "max_test_fix_cycles".to_string(),
            JsonSchema::Number {
                description: Some(
                    "Optional: maximum test→fix cycles before giving up (default 5)."
                        .to_string(),
                ),
            },
        ),
        (
            ...
            JsonSchema::String {
                description: Some(
                    "Optional: project root directory for pipeline documents (default '.')." .to_string(),
                ),
            },
        ),
    ]);
    ...
    ToolSpec::Function(ResponsesApiTool {
        name: "feature_pipeline".to_string(),
        description: "Initialize a TDD-driven feature development pipeline. \
            Parses an implementation plan into features and generates an execution plan \
            with 8 phases per feature: SpecGeneration → TestWrite → TestReview → Implement → \
            TestRun → FixIssues (cycle) → ReviewCommit → DocUpdate. \
            Each phase runs as a WiggumLoop with isolated context and completion signals."
            .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
            required: Some(vec!["implementation_plan".to_string()]),
            additional_properties: Some(false.into()),
        },
    })
}
...
pub(crate) fn build_specs(
    ...
    use crate::tools::handlers::TaskDispatchHandler;
    ...
    // ── Swarm tools (task_dispatch + create_subagent) ──
    {
        ...
        builder.register_handler("task_dispatch", task_dispatch_handler);
        builder.register_handler("create_subagent", create_subagent_handler);
