...
impl ThreadManager {
    pub fn new(
        ...
        Self::new_with_provider(codex_home, auth_manager, session_source, None)
    }

    /// Construct a `ThreadManager` with an optional custom model provider.
    ///
    /// When `provider` is `Some`, the `ModelsManager` will query that
    /// provider's `/models` endpoint instead of the default OpenAI endpoint.
    pub fn new_with_provider(
        ...
        let models_manager = if let Some(provider) = provider {
            ...
            Arc::new(ModelsManager::new(codex_home, auth_manager.clone()))
        };
        Self {
