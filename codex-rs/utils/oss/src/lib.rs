...
use codex_core::LMSTUDIO_LOCAL_PROVIDER_ID;
use codex_core::SYNTHETIC_PROVIDER_ID;
...
pub fn get_default_model_for_oss_provider(provider_id: &str) -> Option<&'static str> {
    match provider_id {
        ...
        LMSTUDIO_LOCAL_PROVIDER_ID => Some(codex_lmstudio::DEFAULT_OSS_MODEL),
        OLLAMA_OSS_PROVIDER_ID => Some(codex_ollama::DEFAULT_OSS_MODEL),
        SYNTHETIC_PROVIDER_ID => Some("claude-sonnet-4-20250514"),
        OPENROUTER_PROVIDER_ID => Some("google/gemini-2.5-flash"),
        _ => None,
    }
}
