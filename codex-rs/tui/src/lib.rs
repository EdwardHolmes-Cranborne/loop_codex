...
pub async fn run_main(
    ...
    // Load any previously saved provider API keys from ~/.codex/provider_keys.env
    // so they're available for the provider flag checks below.
    provider_keys::load_provider_keys();

    let model_provider_override = if cli.local {
        Some(codex_core::LMSTUDIO_LOCAL_PROVIDER_ID.to_string())
    } else if cli.synthetic {
        provider_keys::ensure_api_key_available(
            "SYNTHETIC_API_KEY",
            "Synthetic",
            "https://synthetic.new",
        )?;
        Some(codex_core::SYNTHETIC_PROVIDER_ID.to_string())
        ...
        Some(codex_core::OPENROUTER_PROVIDER_ID.to_string())
    } else if cli.oss {
    ...
    let uses_custom_provider = cli.oss || cli.local || cli.synthetic || cli.openrouter;

    // When using a custom provider flag, let the bootstrapper pick the model based on selected provider
    let model = if let Some(model) = &cli.model {
        Some(model.clone())
    } else if uses_custom_provider {
    ...
    let overrides = ConfigOverrides {
        ...
        show_raw_agent_reasoning: uses_custom_provider.then_some(true),
    ...
    if uses_custom_provider && model_provider_override.is_some() {
        // We're in a custom provider section, so provider_id should be Some
        // Let's handle None case gracefully though just in case
        let provider_id = match model_provider_override.as_ref() {
            Some(id) => id,
            None => {
                error!("Custom provider unexpectedly not set when provider flag is used");
                return Err(std::io::Error::other(
                    "Custom provider not set but provider flag was used",
                ));
            }
        };
        // Only check server readiness for local providers (oss/local);
        // remote providers (synthetic/openrouter) don't need a local server.
        if cli.oss || cli.local {
...
async fn run_ratatui_app(
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        let skip_update_prompt = cli.prompt.as_ref().is_some_and(|prompt| !prompt.is_empty())
            || cli.oss || cli.local || cli.synthetic || cli.openrouter;
