...
impl App {
    ...
    pub async fn run(
        ...
        // If a custom provider is configured (e.g. --synthetic, --openrouter, --local),
        // resolve its ModelProviderInfo so the model picker queries that provider's
        // /models endpoint instead of OpenAI's.
        let custom_provider = {
            let providers = codex_core::built_in_model_providers();
            let provider_info = providers.get(&config.model_provider_id).cloned();
            provider_info.filter(|p| !p.is_openai())
        };
